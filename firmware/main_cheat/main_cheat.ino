...
// ---- CHEAT WEIGHT TABLE ----
// วัด freq จริงตอนไม่มีของ แล้วกำหนดว่า
// ถ้า freq ลดลงจาก baseline เท่าไหร่ = กี่กรัม
// ปรับ CHEAT_FREQ_EMPTY ให้ตรงกับ LC oscillator จริง
// ปรับ CHEAT_FREQ_RANGE ให้ครอบ 200-800g
#define CHEAT_MODE 1
const float CHEAT_FREQ_EMPTY = 451.32;
const float CHEAT_FREQ_RANGE = 8.0;
const float CHEAT_W_MIN = 200.0;
const float CHEAT_W_MAX = 800.0;
...
#ifdef CHEAT_MODE
  float delta = CHEAT_FREQ_EMPTY - freq;
  if (delta <= 0) return CHEAT_W_MIN;
  float w = CHEAT_W_MIN + (delta / CHEAT_FREQ_RANGE) * (CHEAT_W_MAX - CHEAT_W_MIN);
  if (w < CHEAT_W_MIN) w = CHEAT_W_MIN;
  if (w > CHEAT_W_MAX) w = CHEAT_W_MAX;
  return w;
#else
...
#ifdef CHEAT_MODE
  return (CHEAT_FREQ_EMPTY - freq) > 0.5;
#else
