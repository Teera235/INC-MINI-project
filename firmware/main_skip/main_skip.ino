...
void setup() {
  ...
  Serial.println("=== INC AUTOMATION CELL [SKIP MODE] ===");
  Serial.println("Send 's' via Serial to skip current state.");
...
void handleSkip() {
  ...
  Serial.print("SKIP from state ");
  Serial.println((int)state);

  switch (state) {
    case STATE_CAL_EMPTY:
      freqEmpty = 451.0;
      Serial.println("Skip CAL_EMPTY -> CAL_FULL");
      state = STATE_CAL_FULL;
      stateStartTime = millis();
      showText(CLK1, DIO1, TEXT_HI);
      break;
    case STATE_CAL_FULL:
      freqFull = freqEmpty - 6.0;
      Serial.println("Skip CAL_FULL -> WAIT_CARD");
    ...
    case STATE_WAIT_CARD:
      programNum = '2';
      Serial.println("Skip WAIT_CARD -> assume Card B");
      showDigit(CLK1, DIO1, 2);
      shortBeep(150);
      ledBlink(IDX_CARD, 2.0);
    ...
    case STATE_CARD_INVALID:
      ...
      stateStartTime = millis();
      Serial.println("Skip -> WAIT_OBJECT");
      break;
    case STATE_WAIT_OBJECT:
      ...
      stateStartTime = millis();
      Serial.println("Skip WAIT_OBJECT -> REF_DIST");
      break;
    case STATE_REF_DIST:
      state = STATE_SCAN_DIM1;
      Serial.println("Skip REF_DIST -> SCAN_DIM1");
      break;
    case STATE_SCAN_DIM1:
      dimH = 7.5; dimW = dimH; dimL = dimH;
      showCM(CLK1, DIO1, dimH);
      showCM(CLK2, DIO2, dimH);
      showCM(CLK3, DIO3, dimH);
      state = STATE_SCAN_DONE;
      stateStartTime = millis();
      Serial.println("Skip SCAN_DIM1 -> SCAN_DONE (fake 7.5cm)");
      break;
    ...
    case STATE_SCAN_DONE:
      ...
      stateStartTime = millis();
      Serial.println("Skip SCAN_DONE -> TRANSFER");
      break;
    case STATE_TRANSFER:
      analogWrite(PIN_MOTOR_ENA, 0);
      measuredWeight = 500.0;
      ...
      stateStartTime = millis();
      Serial.println("Skip TRANSFER -> WEIGH_DONE (fake 500g)");
      break;
    case STATE_WEIGH_DONE:
      ...
      state = STATE_HEATING;
      stateStartTime = millis();
      Serial.println("Skip WEIGH_DONE -> HEATING");
      break;
    case STATE_HEATING:
      digitalWrite(PIN_RELAY, RELAY_OFF);
      digitalWrite(PIN_LED_KETTLE, LOW);
      ...
      stateStartTime = millis();
      Serial.println("Skip HEATING -> BOIL_COMPLETE");
      break;
    case STATE_BOIL_COMPLETE:
      state = STATE_DONE;
      stateStartTime = millis();
      Serial.println("Skip BOIL_COMPLETE -> DONE");
      break;
    case STATE_DONE:
      Serial.println("Already at DONE");
      break;
    default:
      break;
  }
